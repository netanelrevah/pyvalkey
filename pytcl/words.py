...
from typing import Any, Self, TextIO, TypeVar, dataclass_transform, overload
...
class TCLWordBase:
    ...
    def read(cls, chars: Iterator[str] | str) -> Self:
        ...
        tcl_word_base.origin = iterator.origin
...
class TCLDoubleQuotedWord(TCLWordBase):
    ...
    def _substitute_sub_word(self, word: str, namespace: dict[str, Any]) -> Iterator[str]:
        if word == "":
            yield from ""
        elif word.startswith("$") and word != "$":
            yield from VariableSubstitution.read(iter(word)).substitute_iterator(namespace)
        else:
            yield from word
