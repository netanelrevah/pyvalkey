...
def test_eval_return_table_with_metatable_call_server(r: ValkeyTestClient):
    ...
    assert (
        r.run(
            "eval",
            "local a = {}; setmetatable(a,{__index=function() redis.call('set', 'x', '1') end}) return a",
            1,
            "x",
        )
        == []
...
def test_eval_keys_and_argv_population(r: ValkeyTestClient):
    ...
    assert r.run("eval", "return {KEYS[1],KEYS[2],ARGV[1],ARGV[2]}", 2, "a{t}", "b{t}", "c{t}", "d{t}") == [
        b"a{t}",
        b"b{t}",
        b"c{t}",
        b"d{t}",
...
def test_eval_lua_number_to_redis_integer_conversion(r: ValkeyTestClient):
    ...
    r.run("del", "hash")
    assert r.run("eval", "local foo = redis.pcall('hincrby','hash','field',200000000); return {type(foo),foo}", 0) == [
        b"number",
        200000000,
    ]
...
def test_eval_redis_bulk_to_lua_conversion(r: ValkeyTestClient):
    ...
    r.run("set", "mykey", "myval")
    assert r.run("eval", "local foo = redis.pcall('get',KEYS[1]); return {type(foo),foo}", 1, "mykey") == [
        b"string",
        b"myval",
    ]
...
def test_eval_redis_error_reply_to_lua_conversion(r: ValkeyTestClient):
    ...
    r.run("set", "mykey", "myval")
    assert r.run("eval", "local foo = redis.pcall('incr',KEYS[1]); return {type(foo),foo['err']}", 1, "mykey") == [
        b"table",
...
def test_eval_redis_nil_bulk_reply_to_lua_conversion(r: ValkeyTestClient):
    ...
    r.run("del", "mykey")
    assert r.run("eval", "local foo = redis.pcall('get',KEYS[1]); return {type(foo),foo == false}", 1, "mykey") == [
        b"boolean",
        1,
    ]
